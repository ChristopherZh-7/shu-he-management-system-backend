...
CHECK_ITEMS = [
    ...
    "数据库远程连接暴露",
    "权限分配不合理",
    " HTTP.sys 远程代码执行漏洞（相关补丁缺失）",  # 前面有空格
    "存储型跨站脚本",
    " SNMP 使用默认团体字符串",  # 前面有空格
    "任意用户密码修改/重置（未授权）",
    " SSRF服务器端请求伪造",  # 前面有空格
    " XML外部实体注入",  # 前面有空格
    ...
    " phpinfo 页面泄露",  # 前面有空格
    "源码泄漏",
    " SSL 3.0 POODLE 攻击（CVE-2014-3566）",  # 前面有空格
    "链接注入漏洞",
    "验证码失效",
    "管理后台泄漏",
    "备份文件泄漏",
    ...
    try:
        ...
        for item in CHECK_ITEMS:
            ...
            if matches:
                ...
                normalized_name = re.sub(r'[\-/\(\)（）\s]+', '_', item.strip())
