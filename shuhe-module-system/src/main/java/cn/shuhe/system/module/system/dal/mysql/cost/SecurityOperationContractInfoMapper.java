...
public interface SecurityOperationContractInfoMapper {
     ...
     * 查询用户参与的所有安全运营合同
     * 
     * 逻辑（已修正 v4 - 直接从 security_operation_contract 读取费用）：
     * 1. 从 security_operation_member 表查询用户的实际参与记录
     * 2. 根据 member_type 确定是管理人员(1)还是驻场人员(2)
     * 3. 管理人员获得管理费份额，驻场人员获得驻场费份额
     * 4. 费用直接从 security_operation_contract 表的 management_fee 和 onsite_fee 字段读取
     * 5. 按同一合同下同类型成员人数平均分配
     * 
     * 关联路径：
     * security_operation_member.so_contract_id 
     *   → security_operation_contract.id（直接读取 management_fee / onsite_fee）
    ...
    @Select("SELECT " +
            ...
            // 直接从 security_operation_contract 表读取管理费和驻场费
            "  COALESCE(soc.management_fee, 0) as managementFee, " +
            "  COALESCE(soc.onsite_fee, 0) as onsiteFee, " +
            // 统计该合同下同类型成员数量（在岗状态）
    ...
    @Select("SELECT " +
            ...
            "  soc.contract_no as contractNo, " +
            "  soc.customer_name as customerName, " +
            // 直接从 security_operation_contract 表读取费用
            "  COALESCE(soc.management_fee, 0) as managementFee, " +
            "  COALESCE(soc.onsite_fee, 0) as onsiteFee " +
            "FROM security_operation_member som " +
            "LEFT JOIN security_operation_site sos ON sos.id = som.site_id AND sos.deleted = 0 " +
