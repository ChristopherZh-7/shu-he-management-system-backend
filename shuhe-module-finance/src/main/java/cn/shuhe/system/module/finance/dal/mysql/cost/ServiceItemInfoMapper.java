...
public interface ServiceItemInfoMapper {
     ...
     * 
     * 分配金额来源：finance_allocation 表（Level 3，按 service_item_id 匹配）
     */
    @Select("SELECT " +
            "  pr.id as roundId, " +
            ...
            "LEFT JOIN finance_allocation fa ON fa.service_item_id = pr.service_item_id " +
            "  AND fa.allocation_level = 3 AND fa.deleted = 0 " +
            "WHERE pr.deleted = 0 " +
            "  AND pr.status = 2 " +
    ...
    /**
     * 查询执行人已完成的服务项（分配金额来源：finance_allocation Level 3）
     */
    @Select("SELECT " +
            ...
            "FROM project_info pi " +
            "LEFT JOIN finance_allocation fa ON fa.service_item_id = pi.id " +
            "  AND fa.allocation_level = 3 AND fa.deleted = 0 " +
