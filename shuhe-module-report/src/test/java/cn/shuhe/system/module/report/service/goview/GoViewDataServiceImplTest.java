...
import cn.shuhe.system.module.report.controller.admin.goview.vo.data.GoViewDataRespVO;
...
public class GoViewDataServiceImplTest extends BaseDbUnitTest {
    ...
    public void testGetDataBySQL() {
        ...
        when(sqlRowSet.getObject("name")).thenReturn("戍合").thenReturn("戍合");
        ...
        assertEquals("戍合", dataBySQL.getSource().get(0).get("name"));
        assertEquals(2, dataBySQL.getSource().get(1).size());
        assertEquals(2L, dataBySQL.getSource().get(1).get("id"));
        assertEquals("戍合", dataBySQL.getSource().get(1).get("name"));
    }
